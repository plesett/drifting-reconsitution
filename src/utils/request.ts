...
export class Request {
	...
	static async request(opts: Options) {
		...
		// token存在
		let options = Object.assign(opts, { header: { 'Authorization': `Bearer ${this.getToken()}` } })
		//  Taro.request 请求	
		...
		// 登陆失效 
		if (res.data.code === 99999) { await this.login(); return this.request(options) }
	...
	static getApi(opt: { url: string, data:any, method: Methods }) {
		// 添加前缀
		opt.url = `${MAINHOST}${opt.url}`
